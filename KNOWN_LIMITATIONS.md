...
- Semantic consistency warnings are advisory. They depend on the backend consistency routes and do not block event creation in v0.
- AI DLQ triage is optional and depends on the organization AI provider. Deterministic DLQ grouping should remain useful when AI is disabled.
- The web Observability page can fall back to grouping legacy flat DLQ rows locally, but AI triage requires the grouped DLQ API routes.
...
- Trackboard Guard's SQLite DLQ is not yet connected to the control-plane AI triage workflow. A Guard export/import or forwarding bridge is still planned.
